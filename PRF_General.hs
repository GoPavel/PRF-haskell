...
class Prj2 b r where
  prj2 :: a -> b -> r

-- a -> b -> b
instance Prj2 b b where
  prj2 x y = y

-- a -> b -> r ==> a -> b -> a -> r
instance (Prj2 b r) => Prj2 b (a -> r) where
...
class R c r where
  r :: (x -> r) -> (Int -> c -> x -> r) -> Int -> x -> r

-- (x -> c) -> (y -> c -> x -> c) -> y -> x -> c
instance R Int Int where
  r f g y x | y == 0     = f x
...
-- Example dec
ex4 :: Int -> Int
ex4 = c2 (r z prj1) prj1 prj1

-- f :: a -> r
-- g :: i

-- (a -> b -> a) ==> (a -> b -> c -> a)
-- instance (Prj1 a r) => Prj1 b (a -> r) where
--   prj1 x = x

-- prj1' x = x :: a -> a
-- prj1'' x y = x :: a -> (b -> a)
-- prj1''' x y z = x :: a -> (b -> (c -> a))
